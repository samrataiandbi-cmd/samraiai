	...
	return (
		<div className="relative">
			...
			<div className="h-28 sm:h-32 overflow-hidden relative">
				...
				>
					{testimonials.map((testimonial, index) => (
						<div key={index} className="h-28 sm:h-32 flex flex-col justify-center">
							<div className="flex items-center mb-2">
								<span className="text-yellow-300 font-semibold text-xs sm:text-sm mr-2 sm:mr-3">{testimonial.company}</span>
								<div className="flex">
							...
							<p className="text-slate-300 text-xs sm:text-sm leading-relaxed">"{testimonial.text}"</p>
						</div>
					))}
				</div>
			</div>
			...
			<div className="flex items-center justify-between mt-3 sm:mt-4">
				{/* Previous Button */}
				<button onClick={goToPrev} className="text-slate-400 hover:text-yellow-400 transition-colors p-1" aria-label="Previous testimonial">
				...
				{/* Next Button */}
				<button onClick={goToNext} className="text-slate-400 hover:text-yellow-400 transition-colors p-1" aria-label="Next testimonial">
					<svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
