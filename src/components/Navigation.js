...
export default function Navigation() {
	...
	const handleNavClick = (e, href) => {
		...
		if (targetElement) {
			const offsetTop = targetElement.getBoundingClientRect().top + window.scrollY - 80; // Account for fixed navbar height
			window.scrollTo({
				top: offsetTop,
				behavior: "smooth",
			});
		...
		<nav className="fixed top-0 w-full z-50 bg-red-950/90 backdrop-blur-xl border-b border-white/10 shadow-lg">
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="flex justify-between items-center py-3 sm:py-4">
					...
					<div className="hidden md:flex items-center space-x-8">
						{navLinks.map((link) => {
							...
							if (link.label === "Courses & Batches") {
								return (
									<div key={link.href} className="relative group" aria-haspopup="true">
										<button
										...
										>
											...
											<svg className="w-3 h-3 text-white/60" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
												<path
													fillRule="evenodd"
					...
					<button
						onClick={toggleMenu}
						className="md:hidden p-2 text-white hover:text-orange-400 transition-colors focus:outline-none focus:ring-2 focus:ring-orange-400 rounded-lg"
						aria-label="Toggle mobile menu"
						aria-expanded={isMenuOpen}
					>
						<svg
							className={`w-7 h-7 transform transition-transform duration-200 ${isMenuOpen ? "rotate-90" : ""}`}
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					<div className="pb-4 pt-2 border-t border-white/10">
						<div className="flex flex-col space-y-1">
									...
									return (
										<div key={link.href} className="flex flex-col space-y-1">
											<a
												href="#courses"
												onClick={(e) => handleNavClick(e, "#courses")}
												className="text-white/80 hover:text-orange-400 active:text-orange-500 transition-colors py-3 px-4 rounded-lg hover:bg-white/5 active:bg-white/10 cursor-pointer text-base"
											>
												Courses
											</a>
											<a
							...
							<a
								...
								onClick={(e) => handleNavClick(e, "#start")}
								role="button"
								className="w-full block bg-gradient-to-r from-orange-500 to-amber-500 text-white px-6 py-3 rounded-full font-semibold text-center mt-4 cursor-pointer shadow-md hover:shadow-lg active:shadow-sm transition-all text-base"
							>
								Get Started
							</a>
						</div>
