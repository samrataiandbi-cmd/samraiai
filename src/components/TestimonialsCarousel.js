					...
					>
						{getCurrentSlideTestimonials().map((testimonial, index) => (
							<motion.div
								...
								className="testimonial-card p-6 rounded-2xl transition-all duration-300 shadow-xl max-w-sm mx-auto w-full"
							>
								<div className="flex items-center mb-4">
									<div className="text-3xl mr-3">{testimonial.image}</div>
									<div>
										<h3 className="font-bold text-white text-base drop-shadow-sm">{testimonial.name}</h3>
										<p className="text-orange-300 font-semibold text-sm drop-shadow-sm">{testimonial.role}</p>
										<p className="text-gray-900 text-xs drop-shadow-sm">{testimonial.prevRole}</p>
									</div>
								</div>
								<p className="text-gray-900 leading-relaxed italic text-sm drop-shadow-sm line-clamp-4">
				...
				{Array.from({ length: totalSlides }).map((_, slideIndex) => (
					<button
						...
						className={`w-3 h-3 rounded-full transition-all duration-300 ${
							slideIndex === currentSlide ? "bg-orange-400 w-8" : "bg-white/30 hover:bg-white/50"
						}`}
